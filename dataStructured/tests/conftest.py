...
def _stripe_dummy_key(monkeypatch):
    ...
    but the helpers' `_ensure_key()` still runs and requires `STRIPE_SECRET`
    ...
    """
    if not os.environ.get("STRIPE_SECRET", "").startswith("sk_test_"):
        monkeypatch.setenv("STRIPE_SECRET", "dummy_unit_test_key_not_real")
