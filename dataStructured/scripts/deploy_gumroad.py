#!/usr/bin/env python3
"""
One-shot Gumroad deployment for new-fmcsa-carrier-leads-2026-05.
...
SPEC_FILE = BASE / "state/products/new-fmcsa-carrier-leads-2026-05/spec.json"
LAUNCH_REPORT = BASE / "state/products/new-fmcsa-carrier-leads-2026-05/launch-report.json"
ASSET_PATH = BASE / "state/datasets/new-fmcsa-carrier-leads-2026-05.csv"
...
def run(playwright):
    ...
    try:
        ...
        # Maybe we're already redirected elsewhere after login
        current = page.url
        print(f"   Login redirect: {current}")
        if "gumroad.com" not in current or "login" in current:
            page.screenshot(path="/tmp/gumroad_login_fail.png")
            print("   ERROR: Login failed — screenshot at /tmp/gumroad_login_fail.png")
            browser.close()
            sys.exit(1)
    ...
    page.screenshot(path="/tmp/gumroad_new_product.png")
    ...
    if not submitted:
        ...
        page.screenshot(path="/tmp/gumroad_submit_fail.png")
        browser.close()
        sys.exit(1)
    ...
    page.screenshot(path="/tmp/gumroad_after_create.png")
    ...
    try:
        ...
        page.screenshot(path="/tmp/gumroad_upload_fail.png")
        browser.close()
        sys.exit(1)
    ...
    if not upload_confirmed:
        ...
        page.screenshot(path="/tmp/gumroad_upload_incomplete.png")
        browser.close()
        sys.exit(1)
    ...
    if not published:
        # Fall back to a generic Save only if no explicit Publish control exists.
    ...
    page.screenshot(path="/tmp/gumroad_final.png")

    # Confirm the product is actually live (an Unpublish control / published
    # badge appears once public). Fail loudly if we cannot confirm.
    ...
    # Find the REAL public permalink. The /products/<id>/edit token is NOT the
    # public /l/<permalink> slug, so we must read the actual permalink rather than
    # synthesize one. Try in-page links first, then the Share tab.
