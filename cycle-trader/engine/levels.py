...
purpose is reproducing frozen gates. These functions run in their own
pinned context: prec=34 (IEEE decimal128, ~3x the 10 digits actually
needed), ROUND_HALF_EVEN, and default traps, so a caller cannot perturb a
level by lowering precision, changing the rounding mode, or arming the
Inexact trap.
...
#: Pinned arithmetic context — see the module docstring.
